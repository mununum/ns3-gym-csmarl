...
NS_LOG_COMPONENT_DEFINE ("GymEnv");

NS_OBJECT_ENSURE_REGISTERED (GymEnv);
...
class GymAgent : public Object
{
  friend class GymEnv;

public:
  GymAgent (Ptr<Node> node)
      : m_node (node),
        m_txPktNum (0),
        m_txPktNumLastVal (0),
        m_txPktNumEwma (0.0),
...
GymEnv::GymEnv (NodeContainer &srcNodes, const MyConfig &config)
{
  NS_LOG_FUNCTION (this);

  m_stepTime = Seconds (config.stepTime);

  if (config.algorithm == "80211")
    m_algorithm = IEEE80211;
  else if (config.algorithm == "odcf")
    m_algorithm = O_DCF;
  else if (config.algorithm == "rl")
    m_algorithm = RL;
  else
  ...
  for (auto it = srcNodes.Begin (); it != srcNodes.End (); it++)
    m_agents.push_back (CreateObject<GymAgent> (*it));
  ...
  Simulator::Schedule (Seconds (0.0), &GymEnv::ScheduleNextStateRead, this);
}
...
TypeId
GymEnv::GetTypeId (void)
{
  static TypeId tid = TypeId ("GymEnv")
                          .SetParent<OpenGymEnv> ()
                          .SetGroupName ("OpenGym")
                          .AddConstructor<GymEnv> ();
  return tid;
}
...
GymEnv::ScheduleNextStateRead ()
{
  ...
  Simulator::Schedule (m_stepTime, &GymEnv::ScheduleNextStateRead, this);
}
...
uint32_t
GymEnv::GetQueueLength (Ptr<Node> node)
{
...
void
GymEnv::SetCw (Ptr<Node> node, uint32_t cwValue)
{
...
void
GymEnv::SetSourceInterval (Ptr<Node> node, Time interval)
{
  NS_ASSERT (m_algorithm == RL);
...
Time
GymEnv::GetSourceInterval (Ptr<Node> node)
{
  if (m_algorithm == IEEE80211)
    {
...
bool
GymEnv::ExecuteActions (Ptr<OpenGymDataContainer> actions)
{
  NS_LOG_FUNCTION (this);
...
void
GymEnv::SrcTxDone (Ptr<GymEnv> entity, uint32_t idx, const WifiMacHeader &hdr)
{
  Packet *packet = (Packet *) hdr.m_packet;

  if (packet)
    {
      Ptr<GymAgent> agent = entity->m_agents[idx];
      agent->m_txPktNum++;
...
void
GymEnv::PhyStateChange (Ptr<GymEnv> entity, uint32_t idx, Time start, Time duration,
                          WifiPhyState state)
{
  Ptr<GymAgent> agent = entity->m_agents[idx];
